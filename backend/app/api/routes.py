...
from app.models.schemas import (
    ...
    StoryboardSceneRevisionRequest,
    PublishRequest,
...
def serialize(p: Project):
    ...
    return {
        ...
        "stage": p.stage,
        "status": p.status,
        "research": j(p.research_json, {}),
        "script": j(p.script_json, {}),
        "storyboard": j(p.storyboard_json, []),
        "scenes": j(p.scenes_json, []),
        "voice": j(p.voice_json, {}),
        "render": j(p.render_json, {}),
        "publish": j(p.publish_json, {}),
...
def public_config(
    ...
    return {
        ...
        "video_defaults": cfg.get("video_defaults", {}),
...
def runtime_settings(
    ...
    return {
        "effective": get_effective_config(session),
...
def update_runtime_settings(
    ...
    put(
        "budgets.target_cost_per_video",
        req.target_cost_per_video,
    )
    put(
        "budgets.hard_stop_on_cap",
        req.hard_stop_on_cap,
    )
    put(
    ...
    put(
        "video_defaults.scene_count",
        req.scene_count,
    )
    put(
        "video_defaults.clip_seconds",
        req.clip_seconds,
    )
    put(
        "content.idea_count",
    ...
    put(
        "provider_options.youtube.privacy_status",
        req.privacy_status,
    )
    put(
        "providers.text",
    ...
    put(
        "providers.renderer",
        req.renderer_provider,
    )
    put(
        "providers.publisher",
        req.publisher_provider,
    )
    put(
        "provider_options.openai.model",
        req.openai_model,
    )
    put(
        "provider_options.runway.model",
        req.runway_model,
    )
    put(
        "provider_options.elevenlabs.voice_id",
        req.elevenlabs_voice_id,
    )
    put(
        "provider_options.elevenlabs.model_id",
        req.elevenlabs_model_id,
    )

    effective = save_overrides(
        session,
        current,
    )
...
def ideas(
    ...
    return svc(session).generate_ideas(
        req.count,
        req.category,
    )
...
def idea_history(
    ...
    return [
        serialize_idea_history(x)
        for x in rows
    ]


@router.post("/ideas/history/{idea_id}/use")
def use_history_idea(
    idea_id: int,
    session: Session = Depends(get_session),
):
    idea = session.get(
    ...
    project = svc(session).create_project(
        idea.title,
        idea.category,
        idea.premise,
    )
...
def generate_storyboard(
    ...
    return serialize(
        svc(session).generate_storyboard(
            project_or_404(
                session,
                pid,
            )
...
def scene_decision(
    ...
    try:
        return serialize(
            svc(session).set_scene_approval(
                project_or_404(
                    session,
                    pid,
                ),
...
@router.post(
    "/projects/{pid}/revise-storyboard-scene"
)
def revise_storyboard_scene(
    pid: int,
    req: StoryboardSceneRevisionRequest,
    session: Session = Depends(get_session),
):
    try:
        return serialize(
            svc(session).revise_storyboard_scene(
                project_or_404(
                    session,
                    pid,
                ),
...
def generate_video(
    ...
    try:
        return serialize(
            svc(session).generate_video_scenes(
                project_or_404(
                    session,
                    pid,
                )
...
@router.post(
    "/projects/{pid}/regenerate-video-scene"
)
def regenerate_video_scene(
    pid: int,
    ...
    try:
        return serialize(
            svc(session).regenerate_video_scene(
                project_or_404(
                    session,
                    pid,
                ),
