...
def generate_video(pid: int, session: Session = Depends(get_session)):
    try:
        return serialize(svc(session).generate_video_scenes(project_or_404(session, pid)))
...
def regenerate_video_scene(pid: int, req: SceneVideoDecision, session: Session = Depends(get_session)):
    try:
        return serialize(svc(session).regenerate_video_scene(project_or_404(session, pid), req.scene_index))
    except (ValueError, BudgetExceeded) as e:
        raise HTTPException(400, str(e))
    except TimeoutError as e:
        raise HTTPException(504, str(e))
