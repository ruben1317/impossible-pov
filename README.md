# IMPOSSIBLE POV Content Studio — V1.3.4 Runway Reliability Patch

A YouTube-only, semi-automatic AI video production studio designed to be operated from an iPhone after deployment.

## What V1.3.4 adds

- Runway prompts are automatically capped below the API 1000-character limit.
- Runway HTTP/task errors now surface the provider response instead of a generic Load failed.
- Video-scene generation persists each completed scene immediately and resumes safely after a failure, avoiding duplicate paid generations on retry.
- Existing economy-hybrid mode and Idea History remain enabled.
